...
class construct(tf.keras.Model):
    ...
    def __init__(self, in_dim, out_dim, d_model, num_layers, backward = True):
        ...
        self.sattention = Attention.layer(d_model)
        # self.input_layernorm = tf.keras.layers.LayerNormalization(axis = -1, epsilon = 1e-8)
        self.in_enrnn = LSTM.layer(d_model, False)
        

        # output - in docoder
        self.output_dense = Dense.layer(d_model)
        self.sattention2 = Attention.layer(d_model)
    ...
    def call(self, inputs, outputs, init_states):
        ...
        in_inputs_ = self.sattention(in_inputs, in_inputs, in_inputs)
        # in_inputs = self.input_layernorm(in_inputs + in_inputs_)
        in_enoutputs = self.in_enrnn(in_inputs_, init_states) # 배치, 윈도우, d_model
 
        #decoder
        out_inputs = self.output_dense(outputs)
        out_inputs_ = self.sattention2(out_inputs, out_inputs, out_inputs)
        out_inputs = self.output_layernorm(out_inputs + out_inputs_)
